...
def main():
    ...
    G = net.generator(z_dim=args.z_dim, width=28, ch=256)
    D = net.discriminator(z_dim=args.z_dim, ch=256)
    ...
    opt_G = optimizers.Adam(alpha=2e-4, beta1=0.5)
    opt_D = optimizers.Adam(alpha=5e-4, beta1=0.5)
    opt_G.setup(G)
    opt_D.setup(D)
